...
export function Header() {
  ...
  return (
    <header className="max-w-full fixed w-screen flex items-center justify-between sm:px-10 lg:px-20 bg-light-green h-20">
      ...
      <nav className="sm:hidden md:flex md:w-2/3 justify-between items-center font-medium">
        ...
        {isHover && (
          <nav className="absolute flex flex-col px-4 py-2 bg-white top-14 left-50 z-50">
        ...
        <button className="bg-transparent text-black border-2 py-[6px] px-5 rounded-lg border-dark-green hover:bg-lime-100">Login</button>
        <button className="bg-dark-green text-white py-2 px-5 rounded-lg border-none hover:opacity-80">Register</button>
      </nav>
    </header>
  );
}